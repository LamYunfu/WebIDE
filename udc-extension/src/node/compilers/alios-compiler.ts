...
export class AliosCompiler {
  ...
  extractHex(
    ...
    let _this = this;
    fs.createReadStream(path.join(this.rootDir.val, dirName, "Install.zip"))
      .pipe(unzip.Parse())
      .on("entry", function (entry) {
        let x: string[] = entry.path.split(".");
        if (x.pop() == "bin") {
          Logger.info("find", "find");
          // let fss = fs.createWriteStream(path.join(this.rootDir.val, dirName, 'helloworld@esp32devkitc.bin'))
          let fss = fs.createWriteStream(
            path.join(rootDir, dirName, "sketch.ino.hex")
          );
          entry.pipe(fss);
          fss.on("close", () => {
            ...
            if (useQueue) {
              ...
              // _this.udc.program_device(path.join(this.rootDir.val, dirName, 'sketch.ino.hex'), "0x10000", _this.devs[index], pid).then(
              //     (res) => {
              //         if (res) {
              //             _this.outputResult("program scc")
              //             return new Promise((resolve) => {
  ...
  async postRoleSrcFile(dirName: string, srcFileDirName: string) {
    let absPath = path.join(this.rootDir.val, dirName, srcFileDirName);
    let farr = fs.readdirSync(absPath);
    let fileList: string[] = [];
    farr.forEach(function (item) {
      let suffix = item.split(".").pop();
      if (
        fs.statSync(path.join(absPath, item)).isFile() &&
        suffix != "hex" &&
        suffix != "zip"
      ) {
        fileList.push(item);
