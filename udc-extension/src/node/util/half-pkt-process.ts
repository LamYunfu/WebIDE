...
处理tcp粘包问题
*/
export class HalfPackProcess extends EventEmitor.EventEmitter {
  constructor() {
    super();
  ...
  putData(inData: Buffer): void {
    ...
    this.currentDataSize =
      this.cursorEnd - this.cursorStart < 0
        ? this.maxSize + this.cursorEnd - this.cursorStart
  ...
  acquireData() {
    ...
    if (this.currentDataSize < 0) {
      Logger.info(
        "illegal size:" +
          `${this.currentDataSize}:${this.cursorStart}:${this.cursorEnd}`
      );
      this.cursorStart = 0;
      this.cursorEnd = 0;
      this.currentDataSize = 0;
    ...
    if (this.currentDataSize < 13) {
      //最小包长
      Logger.info("exit1 size:" + this.currentDataSize);
      return;
    }
    let ctlen =
      (this.cursorStart + 6) % this.maxSize <=
      (this.cursorStart + 11) % this.maxSize //包长字段是否为两段
        ? parseInt(
            this.dataBuffer
              .subarray(
                (this.cursorStart + 6) % this.maxSize,
                (this.cursorStart + 11) % this.maxSize
              )
              .toString("ascii")
          )
        : parseInt(
            this.dataBuffer
              .subarray(this.cursorStart + 6, this.maxSize)
              .toString("ascii") +
              this.dataBuffer
                .subarray(0, (11 + this.cursorStart) % this.maxSize)
                .toString("ascii")
          );
    (this.cursorStart + 6) % this.maxSize <=
    (this.cursorStart + 11) % this.maxSize //包长字段是否为两段
    ...
    let x;
    if (ctlen + 12 > (x = this.currentDataSize)) {
      Logger.info(`exit2:${ctlen}:${this.currentDataSize}`);
      Logger.info(`exit2x:${ctlen}:${x}`);
      return;
    }
    let tmp;
    if (isNaN(ctlen)) {
      Logger.info(
        "!!!!!!!!!!!ctlen is NAN,recovery:" +
          `exit2:${ctlen}:${this.currentDataSize}`
      );
      this.cursorStart = 0;
      this.cursorEnd = 0;
      this.currentDataSize = 0;
    ...
    if (this.cursorStart + ctlen + 13 <= this.maxSize) {
      tmp = this.dataBuffer.subarray(
        this.cursorStart,
        this.cursorStart + ctlen + 13
      );
    } else {
      tmp = Buffer.concat([
        this.dataBuffer.subarray(this.cursorStart, this.dataBuffer.length),
        this.dataBuffer.subarray(
          0,
          (this.cursorStart + ctlen + 13) % this.maxSize
        ),
      ]);
    }
    if (
      tmp.subarray(0, 1).toString("ascii") != "{" ||
      tmp.subarray(tmp.length - 1, tmp.length).toString("ascii") != "}"
    ) {
    ...
    console.log("current:" + this.currentDataSize);
    this.emit("data", tmp);
