...
export namespace DiyAI {
    export interface Props {
        section: { [key: string]: any }
        discriptions: string                //实验描述
        title: string                       //实验标题
        buttons: JSON[]                     //实验的按钮
        config: () => void
        say: (verbose: string) => void
        outputResult: (res: string, types?: string) => void
        initPidQueueInfo(infos: string): Promise<string>        //初始化实验信息，打开实验工作空间
    }
    export interface State {
        codingItems: JSX.Element[]
    }
}
export class DiyAIView extends React.Component<DiyAI.Props, DiyAI.State> {
    pidQueueInfo: { [pid: string]: {} } = {};
    pids: string[] = []
    codingItems: JSX.Element[] = []
    constructor(props: Readonly<DiyAI.Props>) {
        super(props)
    }
    componentWillMount() {
...
DiyAIView.contextType = MyContext